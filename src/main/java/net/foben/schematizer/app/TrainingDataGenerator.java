...
public class TrainingDataGenerator {
	...
	public static void main(String[] args) throws RepositoryException,
		...
		configureParser(con);
		con.add(new File(FILE_ALL_EQUIVALENCES), null, RDFFormat.NQUADS,
				(Resource) null);
		con.add(new File(FILE_SCHEMADATA_LDSPIDER), null, RDFFormat.NQUADS,
				(Resource) null);
		RepositoryResult<Statement> res = con.getStatements((Resource) null,
				new URIImpl(equi), (Value) null, false, (Resource) null);
		...
		while (res.hasNext()) {
			...
			try {
				LabelsCommentsResourceDescriptor sr = new LabelsCommentsResourceDescriptor(
						s, 0, 0, con);
				LabelsCommentsResourceDescriptor or = new LabelsCommentsResourceDescriptor(
						o, 0, 0, con);
