...
public class TrainingDataNegatives {
	...
	if (typesprops == 1)
	    ...
	    candArray = getCandidates(top, FILE_PROPERTIES, repo.getConnection());
	else {
	    repo.close();
    ...
    private static LabelsCommentsResourceDescriptor[] getCandidates(int top, String filename, RepositoryConnection con)
	    throws IOException {
	...
	while ((line = in.readLine()) != null && ++count <= top) {
		...
		t = new LabelsCommentsResourceDescriptor(fields[0], Integer.parseInt(fields[1]),
